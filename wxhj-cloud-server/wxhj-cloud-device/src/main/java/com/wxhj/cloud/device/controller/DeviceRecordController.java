...
public class DeviceRecordController implements DeviceRecordClient {
    ...
    public WebApiReturnResultModel deviceInWeek(@RequestBody @Validated CommonIdRequestDTO commonIdRequest){
        ...
        recordTopicMap.forEach((key,value) ->{ typeList.add(key); });
        //默认一周
        List<DeviceInWeekVO> deviceResponseList = new ArrayList<>();
        for(int i=0;i>-7;i--){
            Date dateTemp = DateUtil.growDateIgnoreHMS(new Date(),i);
            List<ViewDeviceRecordVO> listVOTemp = listVO.stream().filter(q-> DateUtil.getStringDate(q.getReceivedDate(),"yyyy-MM-dd").equals(DateUtil.getStringDate(dateTemp,"yyyy-MM-dd"))).collect(Collectors.toList());
